    ...
    <form className="w-full px-10 md:px-45 pt-5" onSubmit={handleSearch}>
      ...
      {suggestions && (
        <div className="bg-green-200 space-y-5">
          {suggestions.map((suggestion) => <span key={suggestion.id} className="flex flex-row gap-5 hover:bg-green-300 cursor-pointer transition py-2 px-3" onClick={() => handleSuggestions(suggestion.id)}><img src={suggestion.large} width={25} height={25}/>{" "}{suggestion.name}</span>)}
        </div>
      )}
